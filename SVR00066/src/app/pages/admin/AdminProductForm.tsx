...
export function AdminProductForm() {
  ...
  const save = async () => {
    ...
    setSaving(false);
    if (err) setError((err as Error).message ?? 'Save failed.');
    else { setSaved(true); setTimeout(() => navigate('/admin/products'), 900); }
  };
    ...
    <div style={{ padding:'40px 32px', maxWidth:860 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:28, flexWrap:'wrap', gap:12 }}>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => navigate('/admin/products')}>Cancel</button>
