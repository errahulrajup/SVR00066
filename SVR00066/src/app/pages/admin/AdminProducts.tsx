...
export function AdminProducts() {
  ...
  const toggle = async (id: string, field: 'visible' | 'featured', val: boolean) => {
  ...
  return (
    <div style={{ padding:'40px 32px' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:28, flexWrap:'wrap', gap:14 }}>
        ...
        <button className="btn btn-gold" onClick={() => navigate('/admin/products/new')}>+ Add Product</button>
      </div>
          ...
          {filtered.map(p => (
            <div key={p.id} style={{ display:'grid', gridTemplateColumns:'64px 1fr auto', gap:16, alignItems:'center', padding:'16px 20px', background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:'var(--radius-md)', transition:'border-color 0.2s' }}
              ...
              <div style={{ display:'flex', gap:8, flexShrink:0 }}>
                <button className="btn btn-dark btn-sm" onClick={() => navigate(`/admin/products/${p.id}`)}>Edit</button>
